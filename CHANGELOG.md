...
- Eight source-backed rules, growing the catalogue from 56 to 64. This is the first round drawn from
  `UPGRADING`'s **Backward Incompatible Changes** section rather than its Deprecated Functionality
  section - behaviour that silently changed, not APIs marked deprecated. All eight are
  `kind: behavior_change`, taking that kind from 2 rules to 10; no new deprecation rule was added, so
  `list-rules --kind=deprecated` returns the same 34 rules it did in `0.3.6`.
  - `core.str_split_empty_string` (8.2) - `str_split('')` returns `[]` rather than `['']`. The shape
    change is the hazard: `$chars[0]` on an empty input used to be `''` and now raises an undefined-key
    warning and yields `null`, which a `string`-typed return then turns into a `TypeError`.
  - `core.range_argument_validation` (8.3) - one `UPGRADING` entry with nine sub-changes, covered as one
    rule because `range()` is ubiquitous. The rule separates the cases that still run (`E_WARNING`) from
    those that do not (`TypeError`, `ValueError`). Measured: `range('a','e',0.5)` does not truncate a
    character list, it resets both boundaries and yields `[0.0]`; `range('5','z')` produces a 70-character
    list where the old behaviour cast one side to `int`.
  - `core.negative_array_index_append` (8.3) - assigning a negative index `n` to an empty array makes the
    next index `n+1` rather than `0`. Measured, the trigger is the container's own key history rather than
    `count() === 0`: an array emptied with `unset()` does not re-arm it, while a fresh `array_filter()`
    result does. Silent before and after, so it cannot be discovered by running the code.
  - `core.exit_as_function` (8.4) - `exit` and `die` now behave like a function. The sharpest consequence
    is `exit(null)`: it exits `0` silently in weak mode and becomes an uncaught `TypeError` exiting 255 the
    moment a project adopts `strict_types=1`, which makes `exit($x ?? null)` a live hazard mid-migration.
  - `core.readonly_indirect_modification_clone` (8.4) - taking a reference to a readonly property inside
    `__clone()`. The boundary is what makes the rule useful: direct reassignment inside `__clone()` is the
    supported readonly-reinitialisation feature and still works, while `$r = &$this->prop` throws.
  - `core.list_destructuring_non_array` (8.5) - `UPGRADING` exempts `NULL`, and a rule omitting that would
    send agents chasing a warning that does not exist. Measured on 8.4.19 every case is silent today:
    `[$a,$b] = null`, `list($c,$d) = 42` and `[$e] = 'str'` all yield `null`, and destructuring a string
    does not give its first character. Destructuring an object is a different, already-loud `Error`, which
    the rule keeps separate.
  - `core.unrepresentable_numeric_casts` (8.5) - one rule for two adjacent entries sharing a theme.
    `(int) 1.0e30` yields `5076964154930102272` today with no diagnostic, while the numeric string
    `(int) '1.0e30'` saturates to `PHP_INT_MAX` instead: float casts wrap, string casts saturate, and both
    are silent.
  - `core.class_alias_reserved_names` (8.5, the round's one mapped rule) - every other reserved word
    already raises an uncatchable fatal today, so `array` and `callable` are the last two stragglers rather
    than a broad hazard.
- One verification fixture, `catalogue_expansion_r6_findings.php`, proving the new mapping through a real
  child process at two version ranges: zero findings at `testVersion 8.2-8.4` and two at `8.2-8.5`.

### Changed

- Mapping coverage **falls** from 35 of 56 rules (62.5%) to 36 of 64 (56%). Seven of the eight new rules
  carry no mapping. This is the third deliberate breadth-for-depth trade, after `0.3.4`
  and `0.3.6`, and the steepest; every surface that reports coverage states the direction it moved.
- `SNIFF_RULE_MAP` grows from 207 to 208 sniff ids and remains the exact bidirectional inverse of the
  rule-local `verification.phpcompatibility` lists.
- The Agent Skill's console goldens and fifteen CLI golden fixtures were regenerated by running the
  commands. Two counts moved in ways a plain total would not predict, because this is the first
  all-`behavior_change` round: under a single-target 8.2 policy seven of the eight are `not_in_range` while
  `core.str_split_empty_string` is `applicable` - its change landed in 8.2 itself - so the not-in-range
  count moves 16 to 23 and the default-shown count 40 to 41.

### Measured, and worth recording

- **PHPCompatibility is structurally blind to this whole section.** Eighteen candidates from `Backward
  Incompatible Changes` were probed against the CI-pinned analyzer and exactly one produced a finding. The
  analyzer detects whether a symbol exists in a version range; it cannot see a function that still exists
  and now returns something different. `(int) 1.0e30` is the clearest case: nothing flags it, and only a
  rule can tell an agent not to write it.
- **The one mapped sniff is broader than the fact it is mapped to.**
  `Keywords.ForbiddenClassAlias.Found` also fires for `int`, `string`, `bool` and `false` dated to PHP 7.0
  and `never` dated to 8.1. The rule records that its message text must be read to tell the 8.5 case apart,
  and the fixture proves the 8.5 case specifically by producing zero findings at `testVersion 8.2-8.4`.
- **Of the 127 `Backward Incompatible Changes` entries across 8.2-8.5, 36 are Core/Standard.** The
  catalogue covered 2 of those before this round and now covers 11, because eight rules cover nine entries.
  Twenty-five remain uncovered.

### Not included

- Any new analyzer. M3-C (PHPStan) stays deferred and M3-D (Rector) stays dropped.
- Any change to the pinned analyzer version.
- The 25 still-uncovered Core/Standard `Backward Incompatible Changes` entries, and the 91 extension-scoped
  ones. They stay recorded rather than folded in to make the round look complete.
- Any framework pack, auto-fix, target-project write, or network rule fetching.
