...
| Rule registry | schema validation、deterministic ordering、32 條 source-backed PHP 8.2–8.5 規則 | 目前只涵蓋 PHP language / Core / bundled extension |
...
Source checkout 與正式發布的 `v0.3.3` PHAR 都具備 `verify` command。Explicit command shape 為：
...
每筆 finding 都會保留 analyzer 自己的 sniff identifier 原文。本專案 32 條規則中有 24 條——包含整個
...
| **Next：further catalogue and mapping growth** | — | 規劃：mapping coverage 目前仍只涵蓋 32 條規則中的 24 條，因此持續擴充 source-backed PHP rule 與其已驗證 mapping，仍排在已延後的 M3-C PHPStan adapter 與已捨棄的 M3-D Rector adapter 之前 | 僅屬於 catalogue 與 mapping 工作，不引入新的 adapter infrastructure |
...
| `resources/rules/` | 32 個 source-backed seed rule JSON，一條 rule 一個檔案 |
