...
## v0.3.6 release and verification adapter boundary

The published release is `0.3.6`. Implemented there: the `version`, `resolve`, `list-rules`,
`explain`, `doctor` and `verify` commands, the Composer Semver policy resolver, the two-axis
applicability engine, the 56-rule seed catalogue in `resources/rules/`, a CI-built PHAR attached to each
...
mutates the target project. Its committed sniff-to-rule mapping covers 35 of the 56 catalogue rules,
